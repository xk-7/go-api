...
// Struct to hold file/folder information
...
// @Success 200 {object} []FileFolderInfo "文件和文件夹列表"
// @Failure 400 {string} string "Invalid input"
// @Failure 500 {string} string "Internal server error"
// @Router /copy [get]
func copy(c *gin.Context) {
	...
	if buildDir == "" && buildCommand == "" {
		filesAndFolders, err := listFiles(buildOutputDir)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": fmt.Sprintf("读取目录失败: %s", err.Error())})
			return
		}

		c.JSON(http.StatusOK, gin.H{"filesAndFolders": filesAndFolders})
		return
	}
...
// @Accept  json
// @Produce  json
// @Param buildOutputDir query string true "构建输出目录"
// @Success 200 {object} []FileFolderInfo "文件和文件夹列表"
// @Failure 400 {string} string "Invalid input"
// @Failure 500 {string} string "Internal server error"
// @Router /listFiles [get]
func listFiles(buildOutputDir string) ([]FileFolderInfo, error) {
	files, err := os.ReadDir(buildOutputDir)
	if err != nil {
		return nil, err
	}

	var fileList []FileFolderInfo
	for _, file := range files {
		fileInfo := FileFolderInfo{
			Name: file.Name(),
...
func copyFilesToServer(buildOutputDir, server, destinationPath string) error {
	files, err := listFiles(buildOutputDir)
	if err != nil {
		return err
	}
	...
	for _, file := range files {
		fileName := file.Name
		removeExistingFilesCommand := fmt.Sprintf("ssh root@%s 'rm -rvf %s/%s'", server, destinationPath, fileName)
		removeCmd := exec.Command("sh", "-c", removeExistingFilesCommand)
		fmt.Println(removeExistingFilesCommand)
		if err := removeCmd.Run(); err != nil {
			return fmt.Errorf("删除现有文件 %s 失败: %v", fileName, err)
		}
	}
	...
	err = cmd.Run()
	if err != nil {
		return fmt.Errorf("文件复制失败: %v, 输出: %s", err, scpOutput.String())
	}
	return nil
}
...
func copyFile(c *gin.Context) {
	...
	for _, server := range servers {
		...
		fmt.Println(removeExistingFilesCommand)
		if err := removeCmd.Run(); err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": fmt.Sprintf("删除现有文件 %s 到 %s 失败: %s", fileName, server, err.Error())})
			return
		}

		scpCommand := fmt.Sprintf("scp -r %s root@%s:%s", filePath, server, destinationPath)
		cmd := exec.Command("sh", "-c", scpCommand)
...
func getContainerLogs(c *gin.Context) {
	...
	// 返回日志文件的下载链接
	logFileURL := fmt.Sprintf("http://192.168.10.204:8081/downloads/%s_%s.log", serverIp, containerName)
