...
    removeExistingFilesCommand := fmt.Sprintf("ssh root@%s 'rm -rf %s/*'", server, destinationPath)
...
        }
    
    removeExistingFilesCommand := fmt.Sprintf("ssh root@%s 'rm -rvf %s/*'", server, destinationPath)
    fmt.Println(removeExistingFilesCommand)
        removeCmd := exec.Command("sh", "-c", removeExistingFilesCommand)
        if err := removeCmd.Run(); err != nil {
            c.JSON(http.StatusInternalServerError, gin.H{"error": fmt.Sprintf("删除现有文件到 %s 失败: %s", server, err.Error())})
            return
        }
